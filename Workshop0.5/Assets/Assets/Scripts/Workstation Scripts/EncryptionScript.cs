...
public class EncryptionScript : MonoBehaviour
{
    ...
    void increaseLights()
    {
        for (int i = 1; i <= 4; i++)
        {
            if(!transform.Find("Light" + i).GetComponent<SpriteRenderer>().enabled)
            {
                transform.Find("Light" + i).GetComponent<SpriteRenderer>().enabled = true;
                return;
            }
        }
    ...
    void turnOffAllLights()
    {
        for (int i = 1; i <= 4; i++)
        {
            if (transform.Find("Light" + i).GetComponent<SpriteRenderer>().enabled)
            {
                transform.Find("Light" + i).GetComponent<SpriteRenderer>().enabled = false;
            }
        }
    }
